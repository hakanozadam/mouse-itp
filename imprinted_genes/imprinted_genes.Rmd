...
plot_snps_of_gene = function(snp_df, gene){
  ...
  this_plot = plot_grid(this_title, paternal_plot, maternal_plot, this_legend, ncol = 1, rel_heights = c(0.2,1,1, 0.4))
...
snp_pos_plot = plot_grid(snp_plot_4cell, snp_plot8_cell , nrow = 1)
...
plot_example_TE = function(TE_df, fourcell_median, eightcell_median){
  ...
  label_y_positions = TE_df$TE + c(0, 0.05, 0, 0, 0, 0)
  text_df = data.frame(x=c(1, 1.2, 1, 2, 2.2, 2), y = label_y_positions, gene = TE_df$gene) 
  
  p = ggplot(data = TE_df, aes(x = stage, y = TE)) + 
    geom_point( aes(color = type, label = as.character(1:6)), position = position_dodge(width = 0.3), size = 1.8 ) + 
...
plot_example_TE = function(TE_df, fourcell_median, eightcell_median){
  ...
  label_x_positions = TE_df$position + c(0, 0, 0.4, 0.4, 0, 0)
  text_df = data.frame(x=label_x_positions, y = label_y_positions, gene = TE_df$gene) 
  
  p = ggplot(data = TE_df, aes(x = stage, y = TE)) + 
    geom_point( aes(color = type, label = as.character(1:6)), position = position_dodge(width = 0.3), size = 1.8 ) + 
    geom_errorbar(aes(ymin=confidence_lower, ymax=confidence_upper, color = type), 
...
first_row_plot = plot_grid(scatter_plot_of_bootstrapped + theme(legend.position = "none"), 
          fourcell_main_boxplot + TWO_STAT,
          eightcell_main_boxplot + TWO_STAT,
          fourcell_mono_boxplot + TWO_STAT,
          eightcell_mono_boxplot + TWO_STAT, nrow =1)

second_row_plot = plot_grid( snp_pos_plot,  main_example_TE_plot, nrow = 1)

imprinted_plot = plot_grid(first_row_plot, second_row_plot, ncol = 1)

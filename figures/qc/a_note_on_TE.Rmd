...
mybarplot = function(TE_ratios,  this_title = ""){
  TE_df = data.frame(TE_ratio = TE_ratios)
  
  p = ggplot(TE_df, aes(x = TE_ratio)) + 
    geom_histogram(color="white", fill="#bf5700", size = 0.3) + 
    scale_y_log10() + 
    xlim(-3, 3) + 
    #scale_x_discrete(breaks = -3:3, labels = as.character(-3:3) ) + 
    labs(title = this_title, x = "TE Ratio", y = "frequency" ) + 
...
barplot_100_cell = mybarplot(human_TE_ratio_100_3_1, this_title = "100-3 / 100-1")
barplot_monosome = mybarplot(human_TE_ratio_Mono_2_3, this_title = "10M-2 / 10M-3")
barplot_mono_and_100 = mybarplot(human_TE_ratio_Mono_100, this_title = "Monosome / Ribo-Itp")

combined_barplot_pre = plot_grid(barplot_100_cell, barplot_monosome, barplot_mono_and_100 , nrow = 1)



  plot_title =  ggdraw() + 
    draw_label(
      "Translation efficiency ratios",
      fontface = 'plain',
...
combined_barplot = plot_grid(plot_title, combined_barplot_pre, ncol = 1, rel_heights = c(0.1, 1.2)  )  
```


```{r  echo=FALSE,warning=FALSE,message=FALSE}
combined_barplot
save_plot_pdf("histograms_of_TE_ratios.pdf", combined_barplot, width = 5.2, height = 2 )
```
